				 ...
				 if (!Reads[ReadIndex].UP_Far.empty()) {
					 if (Reads[ReadIndex].UP_Far[Reads[ReadIndex].UP_Far.size() - 1].LengthStr + Reads[ReadIndex].CloseEndLength < Reads[ReadIndex].ReadLength) {
						 ...
						 if (Reads[ReadIndex].MatchedD == Plus) CountFarEndPlus++;
									...
									cout <<"After brea" <<"\t"
									<< Reads[ReadIndex].Name << "\t"
									<< Reads[ReadIndex].MatchedD << "\t"
									<< Reads[ReadIndex].UP_Far.size() <<" \t"<< Reads[ReadIndex].UP_Close.size() << "\t"
									<< Reads[ReadIndex].UP_Close.size() +Reads[ReadIndex].UP_Far.size() << "\t"
					    ...
					    for (int CloseIndex = 0; CloseIndex < Reads[ReadIndex].UP_Close.size(); CloseIndex++) {
							 ...
							 for (int FarIndex = Reads[ReadIndex].UP_Far.size() - 1; FarIndex >= 0; FarIndex--) {
								 ...
								 if (Reads[ReadIndex].UP_Far[FarIndex].Direction == Minus) {
									...
									<< Reads[ReadIndex].UP_Close.size() +Reads[ReadIndex].UP_Far.size() << "\t"
									<<Reads[ReadIndex].ReadLength<< "\t |||" 
									<< "F_LengthStr:"<< Reads[ReadIndex].UP_Far[FarIndex].LengthStr << "\t"
									<< "C_LengthStr"<<Reads[ReadIndex].UP_Close[CloseIndex].LengthStr << "\t"
									
									
								    << Reads[ReadIndex].UP_Far[FarIndex].LengthStr + Reads[ReadIndex].UP_Close[CloseIndex].LengthStr << "\t" <<endl;
									
									//<< Reads[ReadIndex].UP_Far[FarIndex].AbsLoc << "\t" << Reads[ReadIndex].UP_Close[CloseIndex].AbsLoc + 1 << endl;
								   cout << ""<<endl;
								   */
